...
/// <summary>
///     Represents a token in the lexer, which encapsulates a symbol type, text, line, column, and associated source
///     document.
/// </summary>
public readonly struct Token(Symbol type, ReadOnlyMemory<char> text, int line, int column, SourceDocument document)
{
    ...
    public static Token Invalid(char c, int line, int column, SourceDocument document)
    {
        return new Token("#invalid", c.ToString().AsMemory(), line, column, document);
    }
    ...
    /// </summary>
    /// <returns>
    ///     A <see cref="SourceSpan" /> representing the starting position of the current text segment, based on the current
    ///     line and column.
    /// </returns>
    /// <remarks>
    ...
    /// </summary>
    /// <returns>
    ///     A <see cref="SourceSpan" /> representing the ending position of the current text segment, calculated based on the
    ///     current line, column, and the length of the text.
    /// </returns>
    /// <remarks>
    ...
    {
        return new SourceSpan(Line, Column + Text.Length - 1);
    }
    ...
    /// </returns>
    /// <remarks>
    ///     This method creates a <see cref="SourceRange" /> object that represents the full range of the text segment within
    ///     the document.
    ///     It uses the starting and ending <see cref="SourceSpan" /> objects to define the range.
    /// </remarks>
    public SourceRange GetRange()
    {
        return new SourceRange(Document, GetSourceSpanStart(), GetSourceSpanEnd());
    }
    ...
    {
        return Equals(Type, other.Type) && Text.Equals(other.Text) && Line == other.Line && Column == other.Column &&
